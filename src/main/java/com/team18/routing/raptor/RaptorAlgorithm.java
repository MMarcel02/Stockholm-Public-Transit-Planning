...
public class RaptorAlgorithm implements Router {
    ...
    public RaptorAlgorithm(RaptorNetwork raptorNetwork) {
        ...
        this.arrivalTimesPerRound = new int[(Config.MAX_RAPTOR_ROUNDS + 1) * totalStops];
        this.bestArrivalTime = new int[totalStops];
        this.walkTimeToDestination = new int[totalStops];
        this.destinationBestArrivalTimePerRound = new int[Config.MAX_RAPTOR_ROUNDS + 1];
        this.destinationPriorStopPerRound = new int[Config.MAX_RAPTOR_ROUNDS + 1];
        this.priorStopPerRound = new int[(Config.MAX_RAPTOR_ROUNDS + 1) * totalStops];
        this.routeTakenPerRound = new int[(Config.MAX_RAPTOR_ROUNDS + 1) * totalStops];
    ...
    public List<RouteStep> getFastestTrip(double latFrom, double lonFrom, double latTo, double lonTo, int startTimeSecondsAfterMidnight) {
        ...
        for (int i = 0; i < totalStops; i++) {
            ...
            if (distFromSouce <= Config.MAX_WALK_DISTANCE_INITIAL_AND_FINAL_METRES) {
                int arrivalTime = startTimeSecondsAfterMidnight + (int) (distFromSouce / Config.WALK_SPEED_MPS);
            ...
            if (distToDestination <= Config.MAX_WALK_DISTANCE_INITIAL_AND_FINAL_METRES) {
                walkTimeToDestination[i] = (int) (distToDestination / Config.WALK_SPEED_MPS);
            }
        }
        ...
        if (directDist <= Config.MAX_WALK_DISTANCE_INITIAL_AND_FINAL_METRES) {
            bestTimeAtDestination = startTimeSecondsAfterMidnight + (int) (directDist / Config.WALK_SPEED_MPS);
        ...
        for (int round = 1; round < Config.MAX_RAPTOR_ROUNDS; round++) {
            ...
            // Tiny optimization, basically if we went through the whole network in under Config.MAX_RAPTOR_ROUNDS, then we can break out early
    ...
    private List<RouteStep> reconstructJourney(double latFrom, double lonFrom, double latTo, double lonTo, 
        ...
        if (lastStopId == -2) {
            int durationSecs = (bestTime - startTimeSecondsAfterMidnight);
            routeSteps.add(new RouteStep(latFrom, lonFrom, latTo, lonTo, durationSecs, startTimeSecondsAfterMidnight));
            return routeSteps;
        }

        // RouteStep for final walk
        int arrivalAtLastStop = arrivalTimesPerRound[(bestRound * totalStops) + lastStopId];
        int finalWalkDuration = (bestTime - arrivalAtLastStop);
        ...
        while (currentStopId != -2 && currentRound > 0) {
            ...
            if (routeTakenId == -1) {
                ...
                int durationSecs = (endTime - startTime);

                step = new RouteStep(fromStop, toStop, durationSecs, startTime);
            } else {
                ...
                // really inefficient but im lazy and we only use it in the gui so i think its fine
                int boardTime = startTime;
                for (Trip trip : raptorRoute.trips) {
                    int tripDep = -1;
                    int tripArr = -1;
                    for (StopTime st : trip.stopTimes) {
                        if (st.stop.id.equals(fromStop.id)) tripDep = st.departureTime;
                        if (st.stop.id.equals(toStop.id)) tripArr = st.arrivalTime;
                    }

                    // looking for the trip that matches what we found 
                    if (tripDep >= startTime && tripArr == endTime) {
                        boardTime = tripDep;
                        break;
                    }
                }

                int waitTimeSecs = boardTime - startTime;
                int tripTimeSecs = endTime - boardTime;

                step = new RouteStep(fromStop, toStop, waitTimeSecs, tripTimeSecs, startTime, raptorRoute);
            }
        ...
        if (currentStopId >= 0) {
            int initialWalkDuration = (arrivalTimesPerRound[currentStopId] - startTimeSecondsAfterMidnight);
    ...
    // THE RANGE STUFF IS NOT FINISHED JUST COPY PASTED THE ABOVE STUFF, NEED TO REFACTOR THIS CLASS ANYWAY
